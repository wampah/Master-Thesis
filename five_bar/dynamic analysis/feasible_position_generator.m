% This code generates random configurations for the 5 bar mechanism using a
% random orientations for the proximal links
%
...
% Defines the orientations of proximal links
...
configurations=1e6;% <--------- Total number of configurations to be found 
...
for i=2:configurations
    ...
    % Random orientations are given to the proximal links
    theta1=2*rand()*pi;
    theta2=-2*rand()*pi;

    % Defines matlab functions for phi and phi_q given theta1 and theta2
    ...
    % Newton Rhapson tries to find a configuration that satisfy the random angles
    while true
...
%% Plots the effector points generated (Robot Workspace)
close all
figure(1);
plot(effs(1,:).',effs(2,:).','*')
%% Writes the orientations and effector positions to Parquet
% Parquet is a little more efficient storing data
